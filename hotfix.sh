...
sync_and_restart_tg() {
    ...
    log "Рестарт tg-bot..."
    $VAST_SSH "pkill -f 'python bot.py' 2>/dev/null || true; sleep 1; cd $REMOTE_APP/tg-bot && TG_BOT_TOKEN=\$TG_BOT_TOKEN BOT_API_URL=http://localhost:8000 nohup python bot.py > $REMOTE_LOGS/tg-bot.log 2>&1 &" 2>/dev/null
    log "tg-bot обновлён"
}
...
sync_and_restart_bot() {
    ...
    $VAST_SSH "pkill -f 'uvicorn app.main:app.*8000' 2>/dev/null || true; sleep 1; cd $REMOTE_APP/bot-service && TRANSCRIBER_URL=http://localhost:8001 DATABASE_URL=\"sqlite+aiosqlite:///\/workspace/transcriber.db\" RECORDINGS_DIR=/workspace/recordings BOT_NAME=\"\${BOT_NAME:-Транскрибатор}\" DISPLAY=:99 nohup python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 > $REMOTE_LOGS/bot.log 2>&1 &" 2>/dev/null
...
sync_and_restart_transcriber() {
    ...
    $VAST_SSH "mkdir -p /workspace/voice_bank; pkill -f 'uvicorn app.main:app.*8001' 2>/dev/null || true; sleep 1; cd $REMOTE_APP/transcriber-service && HF_TOKEN=\$HF_TOKEN DIARIZATION_MODEL=\${DIARIZATION_MODEL:-pyannote/speaker-diarization-community-1} CLUSTERING_THRESHOLD=\${CLUSTERING_THRESHOLD:-0.35} CLUSTERING_FA=\${CLUSTERING_FA:-0.04} CLUSTERING_FB=\${CLUSTERING_FB:-0.9} VOICE_BANK_DIR=\${VOICE_BANK_DIR:-/workspace/voice_bank} VOICE_MATCH_THRESHOLD=\${VOICE_MATCH_THRESHOLD:-0.40} MIN_EMBEDDING_SEGMENT_SEC=\${MIN_EMBEDDING_SEGMENT_SEC:-1.0} nohup python -m uvicorn app.main:app --host 0.0.0.0 --port 8001 > $REMOTE_LOGS/transcriber.log 2>&1 &" 2>/dev/null
